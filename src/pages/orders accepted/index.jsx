...
function MyOrdersAccept() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data.data.length > 0 && !addressId) {
          ...
          console.log('Address ID set to:', response.data.data[0].id); // Log to check the address ID
        }
      } catch (error) {
        console.log(error);
      }
  ...
  return (
    <div style={{overflow: 'hidden'}}>
      <NavLink to={'/'} style={{width: '100%', height: '2100px', overflow: 'hidden', position: 'absolute', backgroundColor: '#00000070', left: 0, top: 0, zIndex: 10000, overflow: 'hidden'}}></NavLink>
