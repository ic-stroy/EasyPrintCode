...
import bannerImage from '../../../layouts/images/author_banner.jpeg'

function AdsSliderMobile2() {
              ...
              {slider.map((slideImage, index) => (
                <div style={{marginBottom: '80px', marginTop: '20px'}} className={`carousel-item ${index === 0 ? 'active' : ''}`} key={index} data-index={index}>
                  <NavLink target='_blank' to={`https://${slideImage.url}`}>
                    <center>
                      <Reveal>
                        <div style={{backgroundImage: `url(${bannerImage})`, backgroundPosition: 'center', backgroundSize: 'cover', width: '98%', height: '100px', borderRadius: '20px', backgroundRepeat: 'no-repeat'}}></div>
                      </Reveal>
                    </center>
                  </NavLink>
                </div>
...
export default AdsSliderMobile2;